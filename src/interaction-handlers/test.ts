...
export class ParseExampleInteractionHandler extends InteractionHandler {
	...
	public async run(interaction: ButtonInteraction) {
		let { giveaway } = interaction.client.manager.jobs[interaction.message.id];
		if (!giveaway) {
	...
	private async editMessage(interaction: ButtonInteraction, embed: EmbedBuilder) {
		//? bug(fixed): if ended at the end and somone press will edit
		try {
			if (!this.container.client.manager.jobs[interaction.message.id]) return;
